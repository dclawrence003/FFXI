...
-- Six or more clients can take longer than a few frames to answer the IPC
-- discovery ping.  Collect for a fixed interval so a brief quiet gap cannot
-- truncate the participant list and turn an all/party warp into a local warp.
local participant_scan_wait = 0.50
...
function get_participants()
    ...
    coroutine.sleep(participant_scan_wait)
