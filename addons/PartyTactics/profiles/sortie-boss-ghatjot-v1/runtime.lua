-- Exact-target tank-first pull orchestration for Ghatjot.
--
...
local function release_damage(self,ctx,reason)
    ...
    alert(ctx,('GHATJOT TANK PULL %s: Tackle took the first lane; all five damage lanes released.'):format(reason),false)
end
...
local function retire(self,ctx,reason,keep_armed)
    ...
    if reason=='claim lost' then
        alert(ctx,'GHATJOT target authority released after claim loss; manual play remains available.',false)
    end
end
...
local function bind(self,ctx,mob,now)
    ...
    if ctx.authorize_encounter(mob.id)~=true then
        ...
        alert(ctx,'GHATJOT tank-first pull could not bind; manual combat remains available and automation will retry.',true)
        return false
    end
    ctx.actions.combat_stop_members(ALL_MEMBERS)
    ...
    self.next_force=now
    alert(ctx,'GHATJOT BOUND: Tackle is being sent first; damage releases on his opening threat action.',false)
    return true
end
    ...
    function self:on_tick(ctx,now)
        ...
        if not self.damage_released then
            ...
            then
                ...
                alert(ctx,'GHATJOT TANK PULL FALLBACK: Tackle was directed first but no claim packet arrived within two seconds. Damage released so the timed route cannot deadlock.',true)
            end
        end
    end
