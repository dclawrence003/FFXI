...
local adapter_path = test_dir..'/../gearswap/adapters/'
    ..'locus-dire-bats-tomb-signet/1.3.0.lua'
...
local function world(name, job)
    ...
    local target = {
        ...
        -- Deliberately contradictory: x/y is 5.2 yalms, while this squared
        -- fallback would look like 30. The adapter must align with LP and
        -- prefer coordinates when both are available.
...
do
    ...
    assert(w.action('opener-arm', {'9001', GENERATION, '0', '2'}),
        'adapter rejected LP-compatible nil-valid_target/x-y target')
...
print('PartyTactics Locus Signet protocol-2 v1.3 adapter tests passed.')
