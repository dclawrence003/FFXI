...
local adapter_path = test_dir..'/../gearswap/adapters/'
    ..'locus-dire-bats-tomb-signet/1.6.0.lua'
...
local function world(name, job)
    ...
    local target = {
        ...
        -- Deliberately contradictory target fields. Version 1.6 leaves the
...
-- A duplicate resume is the loss-repair transaction. It must reassert every
-- profile-owned automatic lane and return a fresh exact local SignetKeeper ACK,
-- not merely replay PartyCombat's operator bit. GearSwap addon IPC is local to
-- GearSwap and therefore must not be mistaken for cross-addon delivery.
do
    ...
    assert(count_contains(w.commands,
        'sk __adapter '..GENERATION..' Smalls 0 suspended 1') == 1,
        'suspend did not acknowledge the local SignetKeeper')
    assert(#w.ipc == 0,
        'adapter attempted addon-scoped IPC instead of the local keeper command')
    assert(w.action('resume', {
        GENERATION, '0', '2', '0', '1', '1',
    }))

    -- Treat the first resume command batch as lost. The exact duplicate must
    -- independently reconstruct every RDM-owned lane and ACK the same cycle.
    clear(w.commands)
    assert(w.action('resume', {
        GENERATION, '0', '2', '0', '1', '1',
    }))
    assert(count_contains(w.commands, 'aws2 on') == 1,
        'duplicate resume did not repair AutoWS2')
    assert(count_contains(w.commands,
        'hb deactivateindoors off; hb disable cure; hb enable na; '
            ..'hb disable buff; hb db off; hb as off; '
            ..'hb as attack off; hb on') == 1,
        'duplicate resume did not repair the complete RDM support baseline')
    assert(count_contains(w.commands, 'pc reconcile on') == 1,
        'duplicate resume did not repair the current operator ON state')
    assert(count_contains(w.commands,
        'sk __adapter '..GENERATION..' Smalls 0 resumed 1') == 1,
        'duplicate resume did not return a fresh local SignetKeeper ACK')
    assert(#w.ipc == 0,
        'duplicate resume regressed to addon-scoped IPC delivery')
end

-- COR has one additional owned lane. Its duplicate repair must restore
-- Roller2 alongside AutoWS2 and the ordered PartyCombat state, then ACK the
-- local keeper just like every other client.
do
    local w = world('Dolomedes', 'COR')
    assert(w.authorize(0, '1'))
    assert(w.action('probe', {GENERATION, '0', '2'}))
    assert(w.ack('sk') and w.ack('jk'))
    assert(w.action('suspend', {GENERATION, '0', '2', '1'}))
    assert(w.action('resume', {
        GENERATION, '0', '2', '0', '1', '1',
    }))
    ...
    assert(count_contains(w.commands, 'aws2 on') == 1)
    assert(count_contains(w.commands, 'r2 on') == 1,
        'duplicate COR resume did not repair Roller2')
    assert(count_contains(w.commands, 'pc reconcile on') == 1)
    assert(count_contains(w.commands,
        'sk __adapter '..GENERATION..' Dolomedes 0 resumed 1') == 1,
        'duplicate COR resume did not ACK the local SignetKeeper')
    assert(#w.ipc == 0)
end
...
print('PartyTactics Locus Signet protocol-2 v1.6 resume-repair adapter tests passed.')
