...
local function world()
    ...
    local commands = {}
    local current = {
        id=1001, index=101, name='Smalls', main_job='RDM', sub_job='WHM',
        status='Idle', buffs={},
    }
    ...
    local function ack(generation, nonce)
        return action('companion-ready', {
            generation or GENERATION, '0', '2', 'sk', PROFILE_ID,
            PROFILE_VERSION, ENGINE_VERSION, SIGNATURE, 'Smalls', 'RDM',
            nonce,
        })
    end
    return {
...
-- A load announcement can arrive before the newly loaded keeper is bound.
-- Its first canonical nonce is only a baseline; the equal companion ACK then
-- proves binding without manufacturing a recovery transaction.
do
    local w = world()
    ...
    assert(count_contains(w.commands,
        ...
        'equal bound ACK did not complete ordinary readiness')
end
...
print('PartyTactics Locus Signet raw-keeper recovery adapter v1.7 tests passed.')
