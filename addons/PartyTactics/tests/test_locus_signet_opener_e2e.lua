...
local function dispatch_one(command)
    for part in tostring(command):gmatch('[^;]+') do
        ...
        elseif fields[1] == 'gs' and fields[2] == 'c'
            and fields[3] == 'ptgs' and fields[4] == 'action'
            and adapter
        then
            local args = {}
            for index = 7, #fields do args[#args + 1] = fields[index] end
            local accepted, reason = adapter.handle_action(
                fields[5], fields[6], args)
            assert(accepted, part..' rejected: '..tostring(reason))
        end
    end
end
...
if setfenv then
    ...
    if loader then setfenv(loader, _G) end
else
    loader, load_error = loadfile(adapter_path, 't', _G)
end
...
local accepted, reason = adapter.handle_action('locus-signet', 'authorize', {
    GENERATION, '0', '2', PROFILE_ID, PROFILE_VERSION, SIGNATURE,
    'Dolomedes', ROSTER, '1', '1',
})
assert(accepted, reason)
flush_commands()
accepted, reason = adapter.handle_action('locus-signet', 'probe', {
    GENERATION, '0', '2',
})
assert(accepted, reason)
flush_commands()
...
assert(count_contains('pt __controller_ready locus-signet '..GENERATION
    ..' 0 2') == 1)
accepted, reason = adapter.handle_action('locus-signet', 'companion-ready', {
...
assert(adapter.handle_action('locus-signet', 'operator', {
    GENERATION, '0', '2', '1', '1',
}))
...
os.clock = real_clock
print('PartyTactics/LocusPuller first-hit end-to-end tests passed.')
