...
local MAINTENANCE_HELPER = {RDM='pstartrdm', GEO='pstartgeo'}
...
-- Thunder request is pending, consume only that helper's automatic `tick`
-- command so it cannot take the first legal cast frame. Typed spells,
...
function M.status()
    ...
    local deferred = deferred_request
        ...
        or 'idle'
    return ('manual-pass-through thunder=%s queue=%s deferred=%s dispatched=%d completed=%d rejected=%d')
