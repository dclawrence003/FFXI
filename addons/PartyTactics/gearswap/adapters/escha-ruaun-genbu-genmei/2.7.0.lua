...
-- next-legal priority request per configured client.
--
...
-- requests, Light creates Thunder IV requests, and a coordinated Savage Blade
-- survives Tackle's current healing action. Only a client with a queued
-- spell/WS consumes its automatic helper tick; COR is polled without owning
-- that tick. Presto remains an immediate fixed action; its completion-driven,
-- fail-open sequencing belongs exclusively to the Genbu runtime. Manual
-- actions are never filtered or cancelled.
...
local QUEUE_JOBS = {COR=true, PLD=true, RDM=true, GEO=true}
local HELPER_TICK_JOBS = {PLD=true, RDM=true, GEO=true}
local MAINTENANCE_HELPER = {
    PLD='pstartpld', RDM='pstartrdm', GEO='pstartgeo',
}
...
local QUEUED = {
    ...
    PLD={
        ['savage-blade']={kind='weaponskill', command='/ws', id=42,
            name='Savage Blade', range=4.5, ttl=7, priority=60},
    },
}
...
local ACTIONS = {
    ...
    DNC = {
        ...
        presto={command='/ja', name='Presto', self_target=true},
...
local RECOVERY = {
    ...
    COR={
        ['recover-rolls']={314, 321}, -- Warlock, Samurai
    },
}
...
local rdm_weight_ignored = false
...
local function clear_request(reason, promote)
    queue_request = nil
    next_request_poll = 0
    if promote and deferred_request then
        local pending = deferred_request
        deferred_request = nil
        if os.clock() < pending.expires then queue_request = pending end
    else
        deferred_request = nil
    end
    return reason
...
local function has_live_deferred()
    if deferred_request and os.clock() < deferred_request.expires then
        return true
    end
    deferred_request = nil
    return false
...
local function ensure_rdm_weight_ignore()
    if rdm_weight_ignored or current_job() ~= 'RDM' then return end
    local info = windower.ffxi.get_info()
    if not info or not info.logged_in or tonumber(info.zone) ~= ZONE then return end
    windower.send_command('hb ignore_debuff always weight')
    rdm_weight_ignored = true
end

local function restore_rdm_weight_baseline()
    if not rdm_weight_ignored then return end
    windower.send_command('hb unignore_debuff always weight')
    rdm_weight_ignored = false
end
...
local function bind_combat(id, arguments, target)
    ...
    return true, job..' priority routing bound to exact Genbu'
end

local function end_combat(id, arguments)
    local job = current_job()
    if not QUEUE_JOBS[job] then
        return false, 'combat end belongs only to a queued Genbu client'
    end
    if not combat_binding then
        return true, job..' priority routing was already released'
    end
    if combat_binding.id ~= id
        or not same_authority(combat_binding, arguments)
    then
        return false, 'combat end does not match the active priority binding'
    end
    combat_binding = nil
    clear_request('combat ended')
    return true, job..' priority routing released'
end
...
local function learned_weaponskill(id)
    local abilities = type(windower.ffxi.get_abilities) == 'function'
        and windower.ffxi.get_abilities() or {}
    local known = type(abilities) == 'table'
        and abilities.weapon_skills or {}
    if known[id] == true or known[id] == 1
        or tonumber(known[id]) == id
    then
        return true
    end
    for key, value in pairs(known) do
        if tonumber(value) == id
            or tonumber(key) == id and (value == true or value == 1)
        then
            return true
        end
    end
...
local function engaged_with(id)
    ...
    local battle = type(windower.ffxi.get_mob_by_target) == 'function'
...
local function request_resource(request)
    local resources = type(res) == 'table'
        and (request.kind == 'spell' and res.spells
            or request.kind == 'weaponskill' and res.weapon_skills
            or res.job_abilities)
        or nil
...
local function request_blocker(request, target)
    ...
    if now >= request.expires then
        clear_request(request.action_name..' request expired', true)
        return 'expired'
    end
    if request.inflight_until then
        if now < request.inflight_until then return 'inflight' end
        request.inflight_until = nil
        if has_live_deferred() then
            clear_request(request.action_name..' result timed out', true)
            return 'backoff'
        end
        if request.attempts >= REQUEST_MAX_ATTEMPTS then
            clear_request('result timeout', true)
            return 'expired'
        end
    ...
    if not resource
        ...
        or request.kind == 'weaponskill'
            and not learned_weaponskill(request.action_id)
    then
        clear_request(request.action_name..' is unavailable', true)
        return 'expired'
    end
    if request.kind == 'weaponskill' then
        local current, live = current_player()
        local tp = tonumber(current and current.tp)
            or tonumber(live and live.vitals and live.vitals.tp) or 0
        if tp < 1000 then return 'tp' end
        if not engaged_with(request.id) then return 'engagement' end
    else
        local get_recasts = request.kind == 'spell'
            and windower.ffxi.get_spell_recasts
            or windower.ffxi.get_ability_recasts
        local recasts = type(get_recasts) == 'function'
            and get_recasts() or {}
        if (tonumber(recasts[resource.recast_id or resource.id]) or 0) >= 1
        then
            return 'recast'
        end
    end
    -- Selindrile's silent_can_use() accepts spell IDs only. Job-ability IDs
    -- overlap that table (Thunder Shot 129, Triple Shot 301), so passing them
    -- there can falsely classify a ready ability as an unavailable spell.
    -- The validated ability resource plus its actual ability-recast table are
    -- authoritative; a transient server rejection remains bounded/retriable.
    if request.kind == 'spell' then
...
local function poll_request()
    ...
    if blocker then return queue_request ~= nil, blocker end
...
local function consume_automatic_tick()
    local job = current_job()
    if not active or not HELPER_TICK_JOBS[job] or not queue_request then
        return false
    end
    local _, state = poll_request()
    if job ~= 'PLD' then return queue_request ~= nil end
    -- Tackle is the primary healer. Preserve the next legal WS edge only
    -- while his current action is busy, the WS is in flight, or this poll
    -- actually dispatched it. Low TP, range, disengagement, recast/backoff,
    -- movement, and other non-actionable states leave Majesty healing live.
    return state == 'dispatched' or state == 'busy' or state == 'inflight'
end
...
local function reserve_request(semantic, id, arguments, target)
    ...
    then
        ...
        elseif queue_request.inflight_until then
            if spec.priority > queue_request.priority then
                if not deferred_request
                    or spec.priority > deferred_request.priority
                then
                    deferred_request = request
                end
                return true, spec.name..' deferred behind '
                    ..queue_request.action_name
            end
...
-- RDM/GEO pause their generic helper while their own magic request exists.
-- PLD yields only on a dispatch, in-flight WS, or currently busy action; TP,
-- range, engagement, and backoff never suppress Majesty healing. COR is
-- polled without consuming its native tick. Manual actions always pass.
function M.user_job_tick()
    return consume_automatic_tick()
end

-- PartyTactics' 0.75-second RDM/GEO maintenance fast lane enters through
-- user_job_self_command rather than user_job_tick. While this exact client's
-- priority request is actionable, consume only that helper's automatic `tick`
-- command so it cannot take the same legal action frame. Typed spells,
-- abilities, every other self command, and every other profile pass through.
function M.user_job_self_command(command_args, event_args)
    local job = current_job()
    local helper = MAINTENANCE_HELPER[job]
    if not active or not queue_request or not helper
        or type(command_args) ~= 'table'
        or #command_args ~= 2
        or type(command_args[1]) ~= 'string'
        or type(command_args[2]) ~= 'string'
        or command_args[1]:lower() ~= helper
        or command_args[2]:lower() ~= 'tick'
    then
        return false
    end
    return consume_automatic_tick()
...
function M.job_aftercast(spell, spell_map, event_args)
    ...
    if spell.interrupted then
        request.inflight_until = nil
        if has_live_deferred() then
            clear_request(request.action_name..' was interrupted', true)
        else
            request.next_attempt = os.clock() + REQUEST_RETRY
        end
        if queue_request == request and (request.attempts >= REQUEST_MAX_ATTEMPTS
        ...
        clear_request(request.action_name..' completed', true)
    end
end

function M.action_event(action)
    local request = queue_request
    -- GearSwap sees the completed Invincible packet independently of the
    -- PartyTactics IPC round trip. Retire a Cure-delayed PLD weaponskill at
    -- this earliest local edge; the runtime sends the same scoped cancel as a
    -- belt-and-suspenders path and a later intended chain may rebind normally.
    if request and request.job == 'PLD'
        and type(action) == 'table'
        and tonumber(action.actor_id) == request.id
        and tonumber(action.category) == 6
        and tonumber(action.param) == 22
    then
        combat_binding = nil
        clear_request('Genbu used Invincible')
        return
    end
    local expected_category = request and (request.kind == 'spell' and 4
        or request.kind == 'weaponskill' and 3 or 6)
    ...
    for _, target in ipairs(action.targets or {}) do
        if tonumber(target.id) == expected_target then
            ...
            if result and FAILURE_MESSAGES[tonumber(result.message)] then
                request.inflight_until = nil
                if has_live_deferred() then
                    clear_request('server rejected '..request.action_name, true)
                else
                    request.next_attempt = os.clock() + REQUEST_RETRY
                end
                if queue_request == request
                    and (request.attempts >= REQUEST_MAX_ATTEMPTS
                    or os.clock() >= request.expires
                ) then
                    clear_request('server rejected '..request.action_name, true)
                end
...
-- Manual actions are unconditionally passed through, including while a
-- priority request is waiting for its next legal opportunity.
...
function M.status()
    local queue = queue_request
        ...
        or 'idle'
    local deferred = deferred_request
        and (deferred_request.action_name..'#'..tostring(deferred_request.id))
        or 'idle'
    return ('manual-pass-through routing=%s queue=%s deferred=%s dispatched=%d completed=%d rejected=%d')
        :format(combat_binding and 'bound' or 'unbound', queue, deferred,
