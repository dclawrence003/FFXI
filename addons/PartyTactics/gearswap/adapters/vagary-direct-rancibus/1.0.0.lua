...
local ACTIONS = {
    ...
    -- These empty maps intentionally permit capability proof on all six
    -- assigned jobs without granting either support job a fight action.
    RDM = {},
    GEO = {},
}
...
local function reserve(semantic, value, generation, epoch_value,
    ...
    if action.require_party_coverage and not expected_result_ids then
        queue_state.rejected = queue_state.rejected + 1
        return false,
            'all six intended party identities are required for AoE coverage'
    end
