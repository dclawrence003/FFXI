-- Cooperative GearSwap adapter for fast Sortie A magic-kill credit.
-- Exact target/action reservations wait for a legal cast slot and never
-- consume or cancel manual input.

local M={
    id='sortie-objective-a-magic-kill-v1',version='1.0.0',
...
function M.pre_tick() poll() return false end
function M.user_job_tick() poll() return false end
