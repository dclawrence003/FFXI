-- Cooperative GearSwap adapter for the Sortie A Liquefaction/Fire method.
...
local ACTIONS={
    ...
    RDM={
        ['red-lotus-blade']={kind='weaponskill',id=34,
            name='Red Lotus Blade',command='/ws',range=3.2,min_tp=1000,
            engaged=true,ttl=7.0,priority=110},
        ['fire-v']={kind='spell',id=148,name='Fire V',command='/ma',
            range=20.4,ttl=10.0,priority=100},
        ['fire-iv']={kind='spell',id=147,name='Fire IV',command='/ma',
            range=20.4,ttl=10.0,priority=90},
        ['fire-iii']={kind='spell',id=146,name='Fire III',command='/ma',
            range=20.4,ttl=10.0,priority=80},
    },
}

local state={authority=nil,pending={},next_poll=0,accepted=0,
    dispatched=0,expired=0,last='idle'}
if rawget(_G,'PARTYTACTICS_SORTIE_A_MAGIC_TEST_MODE')==true then
    M._test_state=state
...
local function reserve(semantic,args)
    ...
    if not index or not exact_target(id,index) then
        return false,'target is not an exact live Abject Acuex'
    end
    if not party_claimed(target) then
        return false,'Abject Acuex is not party claimed'
    end
