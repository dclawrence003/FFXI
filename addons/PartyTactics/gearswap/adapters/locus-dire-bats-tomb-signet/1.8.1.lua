...
local COMPANION_BOOTSTRAP_RETRY_INTERVAL = 3
...
local function clear_local(reason, notify_lost, restore_automatic)
    ...
    state.next_companion_bootstrap_at = 0
...
local function prepare_companion_tuple(generation, epoch)
    ...
    state.next_companion_bootstrap_at = now()
        + COMPANION_BOOTSTRAP_RETRY_INTERVAL
    state.controller_announced = false
end

local function announce_controller_ready(force)
    local authority = state.authority
    -- The exact authorized probe proves the local controller. Companion ACKs
    -- are observability, not a prerequisite for the PartyTactics operator
    -- heartbeat: holding that heartbeat here lets SignetKeeper's control lease
    -- expire before it can authorize the next pull. SignetKeeper still owns the
    -- six-member Signet census and is the sole relay to LocusPuller.
    if not authority or state.keeper_recovery then
        return false
    end
...
local function bootstrap_companions(authority, name, missing_only)
    ...
    local keeper = ('sk armpt %s %d %s %s %s %s %s %s %d %s; '
            ...
            authority.operator_armed and '1' or '0',
            authority.generation, authority.epoch)
    local need_keeper = not missing_only or not state.companion_ready.sk
    local need_puller = name == 'Tackleberry'
        and (not missing_only or not state.companion_ready.lp)
    if need_puller then
        ...
        issue(('lp bindpt %s %d %s %s %s %s %s %s %d %s; '
            ...
            :format(authority.generation, authority.epoch,
                ...
                authority.operator_armed and '1' or '0',
                authority.generation, authority.epoch,
                need_keeper and ('; '..keeper) or ''))
    elseif need_keeper then
        issue(keeper)
    end
    if name == 'Dolomedes'
        and (not missing_only or not state.companion_ready.jk)
    then
...
local function repair_missing_companions()
    local authority = state.authority
    if not state.active or not authority or state.keeper_recovery
        or not in_tomb()
        or now() < (state.next_companion_bootstrap_at or 0)
    then
        return false
    end
    state.next_companion_bootstrap_at = now()
        + COMPANION_BOOTSTRAP_RETRY_INTERVAL
    if companions_are_ready(authority.name) then
        if authority.name == 'Dolomedes' then
            -- JubileeKeeper releases ring2 during a transient PT heartbeat
            -- gap but retains its exact tuple. Its recovered PT state alone
            -- cannot reclaim the slot; reassert this current adapter-owned
            -- tuple so the keeper can complete the two-source reproof.
            issue(('jk armpt %s %d %s %s %s %s %s %s'):format(
                authority.generation, authority.epoch, PROFILE_ID,
                PROFILE_VERSION, ENGINE_VERSION, authority.signature,
                LEADER, ROSTER))
        end
        return true
    end
    request_missing_companion_loads(authority.name)
    -- A lost authorize command must be repaired before the helper's pending
    -- 25-second authority expires. Reassert only this exact generation/epoch,
    -- then rebind and request ACK. SignetKeeper still controls pull release.
    if not state.companion_ready.sk then
        issue(('sk authorize %s %d %s %s %s %s %s %s'):format(
            authority.generation, authority.epoch, LEADER, ROSTER,
            PROFILE_ID, PROFILE_VERSION, ENGINE_VERSION,
            authority.signature))
    end
    if authority.name == 'Dolomedes' and not state.companion_ready.jk then
        issue(('jk authorize %s %d %s %s %s %s %s %s'):format(
            authority.generation, authority.epoch, PROFILE_ID,
            PROFILE_VERSION, ENGINE_VERSION, authority.signature,
            LEADER, ROSTER))
    elseif authority.name == 'Tackleberry'
        and not state.companion_ready.lp
    then
    ...
    return bootstrap_companions(authority, authority.name, true)
end
...
local function probe(arguments)
    ...
    then
        ...
        state.last = ready and 'same controller authority reasserted'
            or 'waiting for controller authority'
    ...
    state.last = ready and 'controller authority ready; companion ACKs diagnostic'
        or 'waiting for controller authority'
    return true, state.last
end
...
function M.status()
    local authority = state.authority
    return ('manual-pass-through authority=%s helpers=sk:%s,lp:%s,jk:%s operator=r%d/%s suspended=%s cycle=%s/%s keeper-recovery=%s opener=%s sustain=%d/%d/%d:%s accepted=%d rejected=%d timeouts=%d last=%s')
        :format(authority and (authority.generation..'/'..authority.epoch)
                or 'none',
            state.companion_ready.sk and 'ack' or 'wait',
            state.companion_ready.lp and 'ack' or 'wait',
            state.companion_ready.jk and 'ack' or 'wait',
