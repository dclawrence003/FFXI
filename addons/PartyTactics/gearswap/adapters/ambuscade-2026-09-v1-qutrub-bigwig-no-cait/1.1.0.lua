...
local ACTIONS = {
    COR = {
        ...
        close={command='/ws', name='Last Stand', target='encounter'},
    },
    DNC = {
        food={command='/item', name='Grape Daifuku', target='self'},
        ['shadow-ni']={command='/ma', name='Utsusemi: Ni', target='self'},
        ['shadow-ichi']={command='/ma', name='Utsusemi: Ichi', target='self'},
        lead={command='/ws', name='Evisceration', target='encounter'},
        ['stun-add']={
    ...
    BRD = {
        ...
        ['shadow-ni']={command='/ma', name='Utsusemi: Ni', target='self'},
        ['shadow-ichi']={command='/ma', name='Utsusemi: Ichi', target='self'},
        middle={command='/ws', name='Savage Blade', target='encounter'},
...
local function exact_add(boss, id)
    ...
    if type(target) ~= 'table'
        or target.name ~= ASTROLOGER and not TORMENTORS[target.name]
...
function M.handle_action(controller, semantic, arguments)
    ...
    elseif action.target == 'add' then
        if not exact_add(boss, subject_id) then
            rejected = rejected + 1
