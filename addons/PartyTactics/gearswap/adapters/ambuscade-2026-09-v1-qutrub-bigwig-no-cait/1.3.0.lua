...
local COPY_IMAGE_BUFFS = {[66]=true,[444]=true,[445]=true,[446]=true}
local AREA_CONTROL = {
    ['sheep-song-add']=true,
    ['geist-wall-add']=true,
    ['jettatura-add']=true,
...
local function has_copy_image()
    for id in pairs(COPY_IMAGE_BUFFS) do
...
function M.handle_action(controller, semantic, arguments)
    ...
    if (semantic == 'shadow-ni' or semantic == 'shadow-ichi')
        and has_copy_image()
    then return true, 'Copy Image already active' end
...
function M.zone_change(...) last_dispatch = {} end
