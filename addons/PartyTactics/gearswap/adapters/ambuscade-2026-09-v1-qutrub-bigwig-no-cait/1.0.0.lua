...
-- no-Cait-Sith September Qutrub/Bozzetto Bigwig profile.  This is a separate,
-- immutable contract: the stable host owns callbacks and each character's
-- normal GearSwap file remains the sole equipment owner.
...
local EXPECTED_SUBJOBS = {
    COR='NIN', PLD='BLU', DNC='NIN', BRD='NIN', RDM='WHM', GEO='WHM',
}
...
local ENTRUST_BUFF_ID = 584
local CLARION_BUFF_ID = 499
local MAJESTY_BUFF_ID = 621
local COPY_IMAGE_BUFFS = {[66]=1, [444]=2, [445]=3, [446]=4}
local REQUIRED_SUBJOB_BLUE_MAGIC = {
    [547]='Cocoon', [575]='Jettatura',
    [584]='Sheep Song', [592]='Blank Gaze', [605]='Geist Wall',
}
local LOWHP_CONTROL_TTL = 6.0
local CAPTURE_CONTROL_TTL = 2.5
...
local SUPPORT_NAMES = {
    Tackleberry=true, Smalls=true, Achoo=true,
}
local ALL_PARTY_NAMES = {
    Dolomedes=true, Tackleberry=true, Kickpuncher=true,
    Barneystinson=true, Smalls=true, Achoo=true,
}
local TACKLE_ONLY = {Tackleberry=true}
local STATUS_PROOF_ACTIONS = {
    cocoon=true, entrust=true, ['indi-wilt']=true,
    ['shadow-ni']=true, ['shadow-ichi']=true,
}
local ADD_ENMITY_ACTIONS = {
    ['blank-gaze-add']=true, ['flash-add']=true,
    ['sheep-song-add']=true, ['geist-wall-add']=true,
    ['jettatura-add']=true,
}
local EXECUTED_ENMITY_MESSAGES = {
    [85]=true, [284]=true, [653]=true, [654]=true,
    [655]=true, [656]=true, [659]=true,
}
...
local ACTIONS = {
    COR = {
        ...
        pull={kind='job_ability', id=131, name='Light Shot', category=6,
            command='/ja', target='enemy', range=12, ttl=8,
            max_attempts=2, priority=90, allow_unclaimed=true,
            subjects=BOSS_ONLY},
    ...
    DNC = {
        ...
        lead={kind='weaponskill', id=25, name='Evisceration', category=3,
            ...
            ttl=8, max_attempts=3, priority=50, subjects=ALL_SUBJECTS,
            offense=true, requires_food=true, requires_shadow=true},
        ['stun-add']={kind='job_ability', id=207,
            name='Violent Flourish', category=14, command='/ja',
            target='enemy', range=3.2, ttl=4, max_attempts=2,
            priority=97, subjects=ALL_SUBJECTS, opportunistic=true,
            mechanic_reaction=true},
    },
    PLD = {
        cocoon={kind='spell', id=547, name='Cocoon', category=4,
            command='/ma', target='self', ttl=10, max_attempts=3,
            priority=96, allow_unclaimed=true, buff_id=93,
            fresh_status=true,
            post_claim_exclusion_radius=25},
        ['blank-gaze-add']={kind='spell', id=592, name='Blank Gaze',
            category=4, command='/ma', target='enemy', range=14,
            ttl=8, max_attempts=2, priority=96, subjects=ADD_ONLY,
            boss_exclusion_radius=25, opportunistic=true},
        ['sheep-song-add']={kind='spell', id=584, name='Sheep Song',
            category=4, command='/ma', target='enemy', range=4,
            ttl=9, max_attempts=2, priority=94, subjects=ADD_ONLY,
            boss_exclusion_radius=25, opportunistic=true},
        ['geist-wall-add']={kind='spell', id=605, name='Geist Wall',
            category=4, command='/ma', target='enemy', range=4,
            ttl=9, max_attempts=2, priority=94, subjects=ADD_ONLY,
            boss_exclusion_radius=25, opportunistic=true},
        ['jettatura-add']={kind='spell', id=575, name='Jettatura',
            category=4, command='/ma', target='enemy', range=7,
            ttl=7, max_attempts=2, priority=95, subjects=ADD_ONLY,
            boss_exclusion_radius=25, opportunistic=true},
    ...
    RDM = {
        ...
        ['silence-boss']={kind='spell', id=59, name='Silence', category=4,
            ...
            opportunistic=true},
        ['slow-ii']={kind='spell', id=79, name='Slow II', category=4,
            command='/ma', target='enemy', range=12, ttl=12,
            max_attempts=3, priority=76, subjects=BOSS_ONLY},
        ['paralyze-ii']={kind='spell', id=80, name='Paralyze II',
            category=4, command='/ma', target='enemy', range=12, ttl=12,
            max_attempts=3, priority=76, subjects=BOSS_ONLY},
        ['dia-iii']={kind='spell', id=25, name='Dia III', category=4,
            command='/ma', target='enemy', range=12, ttl=12,
            max_attempts=3, priority=75, subjects=BOSS_ONLY},
    },
    GEO = {
        entrust={kind='job_ability', id=386, name='Entrust', category=6,
            command='/ja', target='self', ttl=7, max_attempts=2,
            priority=95, allow_unclaimed=true, buff_id=ENTRUST_BUFF_ID,
            fresh_status=true},
        ['indi-wilt']={kind='spell', id=787, name='Indi-Wilt',
            category=4, command='/ma', target='party', range=20.4,
            ttl=10, max_attempts=3, priority=94,
            party_subjects=TACKLE_ONLY, requires_entrust=true,
            subject_buff_id=612, fresh_status=true},
        ...
        ['cure-iv']={kind='spell', id=4, name='Cure IV', category=4,
            command='/ma', target='party', range=20.4, ttl=8,
            max_attempts=2, priority=100, party_subjects=ALL_PARTY_NAMES,
            recovery=true},
        poisona={kind='spell', id=14, name='Poisona', category=4,
            command='/ma', target='party', range=20.4, ttl=7,
            max_attempts=2, priority=100, party_subjects=ALL_PARTY_NAMES,
            recovery=true},
        paralyna={kind='spell', id=15, name='Paralyna', category=4,
            command='/ma', target='party', range=20.4, ttl=7,
            max_attempts=2, priority=100, party_subjects=ALL_PARTY_NAMES,
            recovery=true},
        silena={kind='spell', id=17, name='Silena', category=4,
            command='/ma', target='party', range=20.4, ttl=7,
            max_attempts=2, priority=100, party_subjects=ALL_PARTY_NAMES,
            recovery=true},
        cursna={kind='spell', id=20, name='Cursna', category=4,
            command='/ma', target='party', range=20.4, ttl=7,
            max_attempts=2, priority=100, party_subjects=ALL_PARTY_NAMES,
            recovery=true},
        erase={kind='spell', id=143, name='Erase', category=4,
            command='/ma', target='party', range=20.4, ttl=7,
            max_attempts=2, priority=100, party_subjects=ALL_PARTY_NAMES,
            recovery=true},
    },
...
local capture_guard = nil

if rawget(_G, 'PARTYTACTICS_QUTRUB_NO_CAIT_TEST_MODE') == true then
    M._test_queue = queue
...
local function chat(color, message)
    if type(add_to_chat) == 'function' then
        add_to_chat(color, '[PartyTactics Qutrub no-Cait] '..message)
    end
end
...
local function report_status_confirmed(semantic, authority, subject, action,
    request_token)
    if not STATUS_PROOF_ACTIONS[semantic]
        or type(request_token) ~= 'string'
        or request_token:match('^[a-z0-9][a-z0-9_-]*$') == nil
        or not capability or not authority
        or capability.generation ~= authority.generation
        or capability.epoch ~= authority.epoch
    then
        return false
    end
    local status_subject = subject
    if not action.party_subjects then
        local current, live = local_player()
        local player_id = tonumber(current and current.id)
            or tonumber(live and live.id)
        status_subject = player_id
            and windower.ffxi.get_mob_by_id(player_id) or nil
    end
    local encounter_id = tonumber(authority.encounter_id)
    local encounter_index = tonumber(authority.encounter_index)
    local subject_id = tonumber(status_subject and status_subject.id)
    local subject_index = tonumber(status_subject and status_subject.index)
    if not encounter_id or not encounter_index or encounter_index < 0
        or encounter_index > 65535 or encounter_index % 1 ~= 0
        or not subject_id or subject_id < 1 or subject_id > 4294967295
        or subject_id % 1 ~= 0 or not subject_index or subject_index < 0
        or subject_index > 65535 or subject_index % 1 ~= 0
    then
        return false
    end
    windower.send_command(
        ('pt __adapter_status %s %s %d %s %.0f %d %.0f %d %s')
            :format(M.controller, capability.generation, capability.epoch,
                semantic, encounter_id, encounter_index, subject_id,
                subject_index, request_token))
    return true
end
...
local function subjob_blue_magic_ready(job)
    if job ~= 'PLD' then return true end
    local getter = windower and windower.ffxi
        and windower.ffxi.get_sjob_data or nil
    if type(getter) ~= 'function' then return false end
    local ok, data = pcall(getter)
    local spells = ok and type(data) == 'table' and data.spells or nil
    ...
    for key, value in pairs(spells) do
        ...
        if id and REQUIRED_SUBJOB_BLUE_MAGIC[id] then found[id] = true end
    end
    for id in pairs(REQUIRED_SUBJOB_BLUE_MAGIC) do
        if not found[id] then return false end
    end
    return true
end
...
local function probe(generation, epoch_value, protocol_value)
    ...
    if not subjob_blue_magic_ready(job) then
        return false, 'PLD/BLU active Blue Magic set is incomplete or unavailable'
    end
    if job == 'GEO' then
        local entrust = res and res.job_abilities
            and res.job_abilities[ACTIONS.GEO.entrust.id] or nil
        local recasts = windower.ffxi.get_ability_recasts() or {}
        local remaining = entrust
            and tonumber(recasts[entrust.recast_id]) or nil
        if remaining == nil or remaining >= (tonumber(latency) or 1) then
            return false, 'Entrust must be available and off cooldown at activation'
        end
    end
        ...
        then
            ...
            then
                return {
                    ...
                    buffs=member.buffs or live.buffs,
                }
            end
        end
    end
...
local function outside_boss_exclusion(boss, subject, action)
    local radius = tonumber(action and action.boss_exclusion_radius)
    if not radius and party_claimed(boss) then
        radius = tonumber(action and action.post_claim_exclusion_radius)
    end
    if not radius then return true end
    local current, live = local_player()
    local player_id = tonumber(current and current.id)
        or tonumber(live and live.id)
    local caster = player_id and windower.ffxi.get_mob_by_id(player_id) or nil
    local caster_separation = squared_separation(boss, caster)
    -- Sheep Song and Geist Wall radiate from the caster and Jettatura is a
    -- caster-origin cone.  The selected add's boss distance is therefore not
    -- the safety boundary.  The 25-yalm caster boundary also follows the
    -- fight-specific no-Bigwig-hate guidance for PLD add holders and is more
    -- conservative than every Blue Magic footprint used by this adapter.
    return caster_separation ~= nil and caster_separation > radius * radius
end
...
local function context()
    ...
    if not outside_boss_exclusion(boss, subject, action) then
        clear('Tackle entered the Bigwig exclusion radius', false)
        return nil
    end
    return request, boss, subject, action
end

local function subject_has_buff(subject, wanted)
    for _, buff_id in pairs(subject and subject.buffs or {}) do
        if tonumber(buff_id) == wanted then return true end
    end
    -- Raw get_party/get_mob snapshots do not consistently expose remote buff
    -- IDs. GearSwap's packet-maintained alliance view does, indexed both by
    -- resource id and localized name. Bind Colure Active (612) back to the
    -- exact party entity; spell status 557 is the enemy Attack Down effect,
    -- not an aura-status proof on Tackle.
    for party_index = 1, 3 do
        local group = type(alliance) == 'table' and alliance[party_index] or nil
        for member_index = 1, 6 do
            local member = type(group) == 'table' and group[member_index] or nil
            local mob = type(member) == 'table' and member.mob or nil
            local buffs = type(member) == 'table' and member.buffactive or nil
            if type(mob) == 'table' and type(buffs) == 'table'
                and tonumber(mob.id) == tonumber(subject and subject.id)
                and tonumber(mob.index) == tonumber(subject and subject.index)
                and member.name == subject.name
                and (tonumber(buffs[wanted]) or 0) > 0
            then
                return true
            end
        end
...
local function status_satisfied(action, subject)
    if action.status == 'food' then return has_buff(FOOD_BUFF_ID) end
    if action.buff_id then return has_buff(action.buff_id) end
    if action.subject_buff_id then
        return subject_has_buff(subject, action.subject_buff_id)
    end
...
local function nonbusy_blocker(request, subject, action, action_resource)
    ...
    if action.requires_entrust and not has_buff(ENTRUST_BUFF_ID) then
        return 'waiting for exact Entrust status (buff 584)'
    end
    ...
    if not action.mechanic_reaction and not action.recovery
        and emergency_priority(request.job)
    then
...
local function evaluate()
    ...
    if status_satisfied(action, subject)
        and (not action.fresh_status or request.action_confirmed_at)
    then
        report_status_confirmed(request.semantic, request, subject, action,
...
local function set_capture_control(enabled, encounter_value, generation,
    epoch_value)
    local ok, epoch = authorized(generation, epoch_value)
    local encounter_id = uint32(encounter_value)
    if not ok or not encounter_id then
    ...
    if not enabled then
        if not capture_guard or capture_guard.encounter_id == encounter_id then
            capture_guard = nil
            return true, 'capture hold cleared'
        end
        queue.rejected = queue.rejected + 1
        return false, 'capture hold belongs to another encounter'
    end
    local info = windower.ffxi.get_info()
    local boss = windower.ffxi.get_mob_by_id(encounter_id)
    if not info or not info.logged_in or not AMBUSCADE_ZONES[info.zone]
        or not boss or not exact_entity(boss, BOSS,
            encounter_id, boss.index) or not party_claimed(boss)
    then
        queue.rejected = queue.rejected + 1
        return false, 'live party-claimed Bigwig authority is required'
    end
    capture_guard = {
        zone=info.zone, encounter_id=encounter_id,
        encounter_index=boss.index, generation=generation, epoch=epoch,
        expires=os.clock() + CAPTURE_CONTROL_TTL,
    }
    set_binding(capture_guard, generation, epoch)
    local request = queue.request
    local action = request and ACTIONS[request.job]
        and ACTIONS[request.job][request.semantic] or nil
    if request and action and not action.recovery
        and not ADD_ENMITY_ACTIONS[request.semantic]
    then
        clear('spawn-capture hold', false)
    end
    return true, 'capture hold refreshed'
end
...
local function reserve(semantic, encounter_value, generation, epoch_value,
    ...
    if not outside_boss_exclusion(boss, subject, action) then
        queue.rejected = queue.rejected + 1
        return false, 'Tackle must be more than 25y from Bigwig'
    end
    if action.requires_lowhp_guard
    ...
    if status_satisfied(action, subject) and not action.fresh_status then
        report_status_confirmed(semantic, binding, subject, action,
            request_token)
    ...
    if existing then
        ...
        if existing.inflight_until then
            if not old or not old.preemptible_inflight
                or action.priority <= old.priority
            then
                queue.rejected = queue.rejected + 1
                return false, 'another fixed action is already in flight'
            end
            clear('preempted in-flight cleanup by higher-priority '
                ..semantic, false)
        elseif not old or action.priority <= old.priority then
            queue.rejected = queue.rejected + 1
...
local function cancel_request(encounter_value, generation, epoch_value)
    ...
    if capture_guard and capture_guard.encounter_id == encounter_id then
        capture_guard = nil
    end
...
local function capture_recovery_action(spell)
    local name = spell_name(spell)
    if not name then return false end
    if spell.action_type == 'Item' and RECOVERY_ITEMS[name] then return true end
    if RECOVERY_SPELLS[name] and name ~= 'Cure' then return true end
    if name == 'Healing Waltz' then return true end
    local area = name:match('^Curaga') or name:match('^Cura')
        or name:match('^Divine Waltz')
    if area then return lowest_party_hpp() <= EMERGENCY_HPP end
    local targeted = name == 'Cure' or name == 'Full Cure'
        or name:match('^Cure [IVX]+$') or name:match('^Curing Waltz')
    if not targeted then return false end
    local target = type(spell) == 'table' and spell.target or nil
    local hpp = type(target) == 'table'
        and party_hpp_by_name(target.name) or nil
    return hpp ~= nil and hpp <= EMERGENCY_HPP
end
...
local function blocks_action(spell, phase)
    ...
    if capture_active() and not capture_recovery_action(spell) then return true end
...
function M.handle_action(controller, semantic, arguments)
    ...
    elseif requested == 'capture-on' or requested == 'capture-off' then
        return set_capture_control(requested == 'capture-on',
            args[1], args[2], args[3])
    end
    local token = args[4]
...
function M.action_event(packet)
    ...
    local category_ok = tonumber(packet.category) == action.category
    local param_ok = tonumber(packet.param) == action.id
    ...
    for _, target in ipairs(packet.targets or {}) do
        if tonumber(target.id) == tonumber(request.result_id) then
            ...
            if result and FAILURE_MESSAGES[message]
                and not (ADD_ENMITY_ACTIONS[request.semantic]
                    and EXECUTED_ENMITY_MESSAGES[message])
            then
                request.inflight_until = nil
                request.dispatch_token_until = nil
                request.next_attempt = os.clock() + RETRY_DELAY
            ...
            if action.fresh_status then
                request.action_confirmed_at = os.clock()
                request.inflight_until = nil
                request.dispatch_token_until = nil
                request.next_attempt = os.clock() + RETRY_DELAY
                request.last_result = action.name
                    ..' action confirmed; awaiting exact status'
            else
                queue.completed = queue.completed + 1
