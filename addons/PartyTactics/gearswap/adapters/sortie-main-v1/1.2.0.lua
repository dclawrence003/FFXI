...
local LAST_RESORT_ACTION=51
local LAST_RESORT_RECAST=87
local LAST_RESORT_BUFF=64
local LESHONN_NAME='Leshonn'
...
local state={authority=nil,pending={},next_poll=0,accepted=0,
    ...
    last_resort_pending=false,local_leshonn_guard=false}
...
local function current_sub_job()
    local current,live=current_player()
...
local function local_leshonn_target()
    if not logged_in_sortie() then return false end
    local current,live=current_player()
    local status=current and current.status or live and live.status
    local target
    if status=='Engaged' or status==1 then
        target=windower.ffxi.get_mob_by_target('bt')
    end
    target=target or windower.ffxi.get_mob_by_target('t')
    return type(target)=='table' and target.name==LESHONN_NAME
        and tonumber(target.spawn_type)==16 and target.valid_target==true
...
local function apply_brd_support(mode)
    if current_job()~='BRD' then return end
    if mode=='combat' then
        issue('gs c pstartbrd physical Tackleberry')
    else
        -- The frozen magicboss preset keeps March/Minne/Ballad while its
        -- debuff list is empty.  This is the safe travel/Leshonn baseline.
        issue('gs c pstartbrd magicboss Tackleberry')
    end
end

local function apply_pld_support(mode)
    if current_job()~='PLD' then return end
    if mode=='combat' then
        issue('gs c set AutoBuffMode Auto; gs c set AutoTankMode; '
            ..'gs c unset AutoTankFull; gs c pstartpld locusbats Dolomedes')
    else
        -- manualsc retains the reviewed Majesty-healing loop but is not a
        -- sustained-cooldown preset.  Native tank/buff automation is disabled
        -- so it cannot produce Flash or Sentinel behind the adapter.
        issue('gs c set AutoBuffMode Off; gs c unset AutoTankMode; '
            ..'gs c unset AutoTankFull; gs c pstartpld manualsc Dolomedes')
    end
end

local function apply_safe_support(mode)
    apply_brd_support(mode)
    apply_pld_support(mode)
    if current_job()=='RDM' and mode~='combat' then
        issue('gs c weapons Maxentius')
    end
end
...
local function schedule_mode(mode,delay)
    ...
    if mode~='movement' then state.mazurka_pending=false end
    if mode~='leshonn' then state.last_resort_pending=false end
end
...
local function apply_mode(now)
    ...
    local target_is_leshonn=local_leshonn_target()
    if mode=='combat' then
        issue('cancel '..tostring(MAZURKA_BUFF))
        if job=='COR' then
            issue('r2 roll1 chaos; r2 roll2 samurai; r2 on')
        end
        -- A diversion can announce ordinary combat before target
        -- synchronization reaches BRD/PLD.  Keep the universally safe
        -- package while either client still has live Leshonn selected; the
        -- local observer restores ordinary combat as soon as that target
        -- actually changes.
        apply_safe_support(target_is_leshonn and 'leshonn' or 'combat')
        state.mazurka_pending=false
    elseif mode=='leshonn' then
        issue('cancel '..tostring(MAZURKA_BUFF))
        if job=='COR' then
            issue('r2 roll1 chaos; r2 roll2 samurai; r2 on')
        end
        apply_safe_support('leshonn')
        state.mazurka_pending=false
    elseif mode=='movement' then
        ...
        apply_safe_support('movement')
    end
    state.applied_mode=mode
    if (mode=='leshonn' or mode=='combat') and target_is_leshonn then
        state.local_leshonn_guard=true
    elseif mode~='leshonn' and not target_is_leshonn then
        state.local_leshonn_guard=false
    end
    state.last=mode..' mode applied'
    return true
end

local function try_last_resort()
    if not state.last_resort_pending or state.applied_mode~='leshonn'
        or current_sub_job()~='DRK' or not logged_in_sortie()
    then return false end
    if has_buff(LAST_RESORT_BUFF) then
        state.last_resort_pending=false
        return true
    end
    local abilities=windower.ffxi.get_abilities() or {}
    if not collection_has(abilities.job_abilities or {},LAST_RESORT_ACTION)
    then
        state.last_resort_pending=false
        state.last='Last Resort unavailable'
        return false
    end
    local recasts=windower.ffxi.get_ability_recasts
        and windower.ffxi.get_ability_recasts() or {}
    if (tonumber(recasts[LAST_RESORT_RECAST]) or 0)>0
        or moving or type(midaction)=='function' and midaction()
        or function_true('silent_check_disable')
    then return false end
    windower.chat.input('/ja "Last Resort" <me>')
    state.last_resort_pending=false
    state.last='Last Resort dispatched for confirmed Wind hands'
    return true
end

local function observe_local_leshonn()
    local job=current_job()
    if job~='BRD' and job~='PLD' then return false end
    local observed=local_leshonn_target()
    if observed and not state.local_leshonn_guard then
        apply_safe_support('leshonn')
        state.local_leshonn_guard=true
        state.last='local Leshonn safety preview applied'
    elseif not observed and state.local_leshonn_guard
        and state.applied_mode~='leshonn'
        and state.pending_mode~='leshonn'
    then
        apply_safe_support(state.applied_mode=='combat' and 'combat'
            or 'movement')
        state.local_leshonn_guard=false
        state.last='local Leshonn safety preview released'
    end
    return observed
end
...
local function handle_mode(semantic,args)
    ...
    if #args<3 or #args>4 or not id
        or not set_authority(args[2],args[3]) or not safe_token(args[4])
    then return false,'invalid mode authority' end
    if semantic=='leshonn-mode' or semantic=='leshonn-wind-mode'
        or semantic=='leshonn-thunder-mode'
    then
        local target=windower.ffxi.get_mob_by_id(id)
        local index=target and tonumber(target.index) or nil
        if not index or not exact_target(id,index)
            or target.name~=LESHONN_NAME
        then return false,'Leshonn mode requires exact live Leshonn' end
    end
    ...
    elseif semantic=='leshonn-mode' then
        schedule_mode('leshonn',0)
        state.last_resort_pending=false
        if current_sub_job()=='DRK' and has_buff(LAST_RESORT_BUFF) then
            issue('cancel '..tostring(LAST_RESORT_BUFF))
        end
    elseif semantic=='leshonn-wind-mode' then
        schedule_mode('leshonn',0)
        if current_sub_job()=='DRK' then state.last_resort_pending=true end
    elseif semantic=='leshonn-thunder-mode' then
        schedule_mode('leshonn',0)
        state.last_resort_pending=false
        if current_sub_job()=='DRK' and has_buff(LAST_RESORT_BUFF) then
            issue('cancel '..tostring(LAST_RESORT_BUFF))
...
function M.handle_action(controller,semantic,args)
    ...
    elseif semantic=='movement-mode' or semantic=='combat-mode'
        or semantic=='leshonn-mode' or semantic=='leshonn-wind-mode'
        or semantic=='leshonn-thunder-mode'
...
function M.user_job_tick()
    poll()
    -- This consumes only the inherited automatic job tick while the exact
    -- local Leshonn target is selected.  Manual actions still pass through
    -- both filters below unchanged; reviewed maintenance ticks continue to
    -- run the safe BRD/PLD helper presets.
    return (current_job()=='BRD' or current_job()=='PLD')
        and local_leshonn_target() or false
end
