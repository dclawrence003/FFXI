...
local ACTIONS = {
    ...
    BRD = {
        ...
        mew={kind='pet_ability', id=522, result_ids={[522]=true,[2449]=true},
            ...
            requires_pet=true, full_pack_result=true},
        retreat={kind='pet_ability', id=89, name='Retreat', category=6,
            command='/pet', target='self', ttl=4, max_attempts=1,
            priority=20, requires_pet=true, opportunistic=true,
            preemptible_inflight=true, self_or_pet_result=true},
    ...
    RDM = {
        ...
        -- Smalls is the redundant /SMN lane.  Summon and Mew deliberately
        -- rank below exact cures, Diaga, Astrologer Dispel, and Silence so a
        -- waiting pet action can never own their action window.
        summon={kind='spell', id=307, name='Cait Sith', category=4,
            command='/ma', target='self', ttl=12, max_attempts=3,
            priority=65, allow_unclaimed=true, status='cait-sith'},
        mew={kind='pet_ability', id=522, result_ids={[522]=true,[2449]=true},
            result_categories={[6]=true,[11]=true}, name='Mewing Lullaby',
            category=11, command='/pet', target='enemy', range=14,
            ttl=12, max_attempts=3, priority=80, subjects=BOSS_ONLY,
            requires_pet=true, full_pack_result=true},
        retreat={kind='pet_ability', id=89, name='Retreat', category=6,
            command='/pet', target='self', ttl=4, max_attempts=1,
            priority=20, requires_pet=true, opportunistic=true,
...
local function reserve(semantic, encounter_value, generation, epoch_value,
    ...
    if action.opportunistic then
        if not action_resource or not learned(action)
            ...
            or action.requires_pet and not cait_sith()
...
function M.action_event(packet)
    ...
    for _, target in ipairs(packet.targets or {}) do
        if tonumber(target.id) == tonumber(request.result_id)
            or action.self_or_pet_result and pet_entity
                and tonumber(target.id) == tonumber(pet_entity.id)
        then
            ...
            if action.full_pack_result then
                local boss = windower.ffxi.get_mob_by_id(request.encounter_id)
