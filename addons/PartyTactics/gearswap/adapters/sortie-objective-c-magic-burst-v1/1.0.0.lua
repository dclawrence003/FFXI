-- Cooperative GearSwap adapter for Sortie C Magic Burst credit.
--
-- Requests are exact-ID, exact-action, short-lived reservations. The adapter
-- never cancels manual input and every hook yields to the native job file.

local M = {
    id='sortie-objective-c-magic-burst-v1',
    version='1.0.0',
    controller='sortie-c-mb',
    protocol=1,
}
...
local ACTIONS = {
    COR = {
        ['savage-blade']={kind='weaponskill', id=42,
            name='Savage Blade', command='/ws', range=3.2,
            min_tp=1000, engaged=true, ttl=3.5, priority=60},
    },
    PLD = {
        ['savage-blade']={kind='weaponskill', id=42,
            name='Savage Blade', command='/ws', range=3.2,
    ...
    DNC = {
        evisceration={kind='weaponskill', id=25,
            name='Evisceration', command='/ws', range=3.2,
            min_tp=1000, engaged=true, ttl=3.5, priority=60},
    },
    BRD = {
        ['savage-blade']={kind='weaponskill', id=42,
    ...
    RDM = {
        thunder={kind='spell', id=164, name='Thunder', command='/ma',
            range=20.4, ttl=3.5, priority=90},
    },
    GEO = {
        thunder={kind='spell', id=164, name='Thunder', command='/ma',
            range=20.4, ttl=3.5, priority=90},
...
if rawget(_G, 'PARTYTACTICS_SORTIE_C_MB_TEST_MODE') == true then
    M._test_state = state
end
...
local function exact_target(id, expected_index)
    ...
    return type(target) == 'table'
        ...
        and TARGET_NAMES[target.name] == true
...
local function poll()
    ...
    for semantic, request in pairs(state.pending) do
        local action = ACTIONS[request.job]
            and ACTIONS[request.job][semantic] or nil
        local why
        if action then
...
local function reserve(semantic, args)
    ...
    if not index or not exact_target(id, index) then
        return false, 'target is not an exact live normal Cachaemic'
    end
    if not party_claimed(target) then
        return false, 'Cachaemic is not party claimed'
    end
...
-- Explicit cooperative hooks: an automated reservation never consumes or
-- rejects the player's pretarget/precast path.
...
function M.status()
    local pending = {}
    for semantic, request in pairs(state.pending) do
        pending[#pending + 1] = semantic..'='
            ..tostring(request.blocker or 'ready')
    end
    table.sort(pending)
