...
local PSTART_PLD_HELPER_VERSION = '1.0.0'

local function pstart_pld_prove_helper(generation, epoch_text, version)
    local epoch = type(epoch_text) == 'string'
        and epoch_text:match('^%d+$') and tonumber(epoch_text) or nil
    if version ~= PSTART_PLD_HELPER_VERSION
    ...
    windower.send_command(('pt __legacy_helper_ready %s PLD %s %d')
        :format(PSTART_PLD_HELPER_VERSION, generation, epoch))
    return true
end
...
local PSTART_PLD_ACTION_LEASE_TIMEOUT = 5
local PSTART_PLD_ACTION_LEASE_BACKOFF = 1.5
...
local PSTART_PLD_NO_CHIVALRY_PROFILES = {
    fishfly=true,
    manualsc=true,
}
...
-- Fishflies counter magic in synchronized bursts and die too quickly for a
-- conservative sustain threshold to be useful.  Do not treat small apparent
-- HP losses from a caster changing max-HP gear as damage: routine recovery
-- starts at 90%, while two genuinely injured members below 95% still trigger
-- an early Majesty cure.  Emergency recovery remains aggressive below 80%.
local PSTART_PLD_FISHFLY_HEAL_POLICY = {
    routine_hpp = 90,
    cluster_hpp = 95,
    cluster_count = 2,
    cluster_mp_floor = 20,
    ...
    conserve_mp_hpp = 40,
    low_mp_hpp = 25,
    conserve_routine_hpp = 85,
    low_mp_routine_hpp = 75,
    cure_interval_high = 1.5,
    cure_interval_mid = 2.5,
    cure_interval_low = 4,
    cure_iv_hpp = 85,
    chivalry_reserve_hpp = 40,
    chivalry_use_hpp = 35,
    chivalry_release_hpp = 55,
}

local function pstart_pld_heal_policy()
    if pstart_pld.profile == 'fishfly' then
        return PSTART_PLD_FISHFLY_HEAL_POLICY
    end
    return (pstart_pld.profile == 'apexcrabs'
            or pstart_pld.profile == 'limbus')
        and PSTART_PLD_CRAB_HEAL_POLICY or PSTART_PLD_DEFAULT_HEAL_POLICY
end
...
-- PartyTactics supplies an explicit maintenance heartbeat in addition to the
-- character file's native user_job_tick. One issued command therefore needs a
-- single local lease shared by both entry points. Aftercast normally releases
-- it; the timeout handles a command rejected before GearSwap receives an
-- aftercast and prevents a permanent deadlock.
local function pstart_pld_claim_action(pending)
    local now = os.clock()
    pending.issued_at = now
    pending.expires_at = now + PSTART_PLD_ACTION_LEASE_TIMEOUT
    pstart_pld.pending = pending
end

local function pstart_pld_action_lease_active()
    local pending = pstart_pld.pending
    if not pending then return false end
    local now = os.clock()
    if now < (pending.expires_at or now) then return true end
    pstart_pld.pending = nil
    pstart_pld.retry_at = now + PSTART_PLD_ACTION_LEASE_BACKOFF
    pstart_pld.dispatch_timeouts = (pstart_pld.dispatch_timeouts or 0) + 1
    pstart_pld.last_action = (pending.spell_name or 'action')
        ..' dispatch unconfirmed; bounded retry armed'
    add_to_chat(123, '[PartyStart PLD] '..pstart_pld.last_action)
    return true
end
...
local function pstart_pld_war_subjob_enmity()
    ...
    if pstart_pld.profile == 'fishfly'
        or pstart_pld.profile == 'manualsc'
    then
        -- Defensive-only presets never apply Warcry, Aggressor, or Berserk.
        -- Fishfly owns opening hate on the pack and needs no Provoke; manualsc
        -- retains Provoke while preserving TP and defensive posture for a
        -- manually coordinated weapon-skill chain.
        ...
        if pstart_pld.profile == 'manualsc' then
            return pstart_pld_use_war_ability(
...
local function pstart_pld_cast_cure(lowest, cluster_injured, reason)
    ...
    local command_delay = pstart_pld.profile == 'fishfly' and 1.5 or 2.5
...
local function pstart_pld_sustain_mp()
    if PSTART_PLD_NO_CHIVALRY_PROFILES[pstart_pld.profile] then
        -- These profiles deliberately leave AutoWS2 hard-Off. Never reserve
        -- or spend their TP for Chivalry, and never re-enable AutoWS2 through
...
local function pstart_pld_action()
    ...
    if pstart_pld_action_lease_active() then return true end
    if midaction() or moving or silent_check_disable()
        or (tickdelay and os.clock() < tickdelay)
        or os.clock() < (pstart_pld.retry_at or 0)
    then
        return false
...
local function pstart_pld_status()
    ...
    add_to_chat(122,
        ('PartyStart PLD sustain: cures %d / lease timeouts %d / Majesty %s / Refresh %s / '
            ..'Ballad %s / Entrust %s / Chivalry %s / AutoWS2 %s')
        :format(
            pstart_pld.cure_count,
            pstart_pld.dispatch_timeouts or 0,
...
function user_job_self_command(commandArgs, eventArgs)
    ...
    elseif requested == 'off' then
        ...
        pstart_pld.dispatch_timeouts = 0
    ...
    then
        ...
        add_to_chat(123,
            ...
            ..'<master|apexbats|locusbats|apexcrabs|limbus|fishfly|manualsc|ambuscade-v1|ambuscade-v2|status|off> <leader>')
    end
end
...
function user_job_tick()
    ...
    if pstart_pld.active and ((tickdelay and os.clock() < tickdelay)
        or os.clock() < (pstart_pld.retry_at or 0))
    then
        -- The explicit PartyTactics tick and native tick share one action
        -- recovery window. Do not let the native loop issue into that window.
        return true
    end
...
function job_aftercast(spell, spellMap, eventArgs)
    ...
    elseif pending and spell and spell.id == pending.spell_id then
        if spell.interrupted then
            ...
            local minimum_retry = (pending.command_delay or 2.5) < 2.5
                and 0.5 or 1
            pstart_pld.retry_at = os.clock()
                + math.max(minimum_retry, pstart_pld_cure_interval()
                    - (pending.command_delay or 2.5))
        end
        pstart_pld.pending = nil
    end
