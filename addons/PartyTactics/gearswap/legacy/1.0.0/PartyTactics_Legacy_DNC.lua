...
local PSTART_DNC_HELPER_VERSION = '1.0.0'

local function pstart_dnc_prove_helper(generation, epoch_text, version)
    local epoch = type(epoch_text) == 'string'
        and epoch_text:match('^%d+$') and tonumber(epoch_text) or nil
    if version ~= PSTART_DNC_HELPER_VERSION
    ...
    windower.send_command(('pt __legacy_helper_ready %s DNC %s %d')
        :format(PSTART_DNC_HELPER_VERSION, generation, epoch))
    return true
end
...
local PSTART_DNC_HEAL_ONLY_PROFILES = {
    fishfly=true,
    tankheal=true,
}
...
local function pstart_dnc_action()
    ...
    local heal_only = PSTART_DNC_HEAL_ONLY_PROFILES[pstart_dnc.profile] == true
    ...
    if heal_only then
        -- Heal-only presets grant DNC only the TP-funded emergency Waltz path:
        -- no Samba, Step, Flourish, or AutoWS. The preset's encounter profile
        -- decides whether the DNC is in melee range.
        return false
    end
...
function user_job_self_command(commandArgs, eventArgs)
    ...
    elseif requested and S{
        'master','apexbats','locusbats','apexcrabs','limbus','fishfly','tankheal','physical','accuracy','magic','safe',
    ...
    then
        ...
        pstart_dnc.autows_paused = false
        tickdelay = 0
        if PSTART_DNC_HEAL_ONLY_PROFILES[requested] then
            add_to_chat(122,
                'PartyStart DNC heal-only: emergency Waltz support is On; '
        ...
        add_to_chat(123,
            ...
            ..'<master|apexbats|locusbats|apexcrabs|limbus|fishfly|tankheal|physical|accuracy|magic|safe|ambuscade-v1|'
...
function user_job_tick()
    ...
    if pstart_dnc.active
        and PSTART_DNC_HEAL_ONLY_PROFILES[pstart_dnc.profile]
    then
