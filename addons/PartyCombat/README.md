...
It is intentionally conservative. Loading the addon does nothing. Normal
`//pc on` authorization still requires a validated runtime policy. An explicit
`//pc force`/`//pc smash` is the deliberate exception: when no fresh policy is
available, it creates a temporary mobile all-attacker policy from
`settings.lua` and attacks the controller's current target. It does not start
healing, buffs, songs, rolls, AutoWS2, or any other PartyStart automation.
...
- `force` explicitly targets and engages the invoking controller's current
  enemy up to the configured 30-yalm safety limit. With a fresh PartyStart
  policy, it preserves that profile's attackers, observers, priority targets,
  and movement mode. Without one, it deliberately falls back to a temporary
  mobile **manual-smash** policy containing every attacker in `settings.lua`.
...
- A fresh load begins with `support-ready No`. `//pc on` remains blocked until
  PartyStart commits a validated profile (or an explicit `//pc policy ...`
  command installs one). `//pc force` and its `//pc smash` alias are explicit
  manual commands and may instead activate `manual-smash Yes`. PartyStart
  stop/unload, `//pc stop`, and zoning all disarm and clear manual smash.
...
- `force` / `smash`: Targets and engages the invoking controller's current
  enemy. If `support-ready Yes`, the active PartyStart policy remains in
  control. If `support-ready No`, all attackers from static `settings.lua` are
  authorized under a temporary mobile manual-smash policy. This is the roaming
  "just attack it" path; it deliberately provides no support automation.
...
For profile-controlled combat, the normal flow is:

1. Apply the desired PartyStart composition and wait for its ready message.
   `//pc status` must say `support-ready Yes`; then use `//pc on` once on
   either Dolomedes or Tackleberry to arm PartyCombat.
...
For unscripted roaming combat, target a living enemy on Dolomedes or
Tackleberry and press Alt+P. When no fresh PartyStart policy exists, the same
`pcombat force` binding visibly reports `Manual smash armed` and authorizes all
six static attackers. Press Alt+O when finished. Ctrl+P is intentionally not
bound because FFXI reserves it for party chat.
...
PartyStart does not initiate engagement. Its composition selects the command
...
`//pstart off` stops the support it owns and invalidates/disarms PartyCombat so
stale combat cannot continue without healing; it still leaves FastFollow
untouched. Reloading PartyStart has the same readiness effect.
PartyStart's unattended `master` (Apex Efts), `apexbats`, `locusbats`, and `apexcrabs`
