...
- A fresh load begins with `support-ready No`. `//pc on` and `//pc force` are
  blocked until PartyStart commits a validated profile (or an explicit
  `//pc policy ...` command installs one). PartyStart stop/unload invalidates
  readiness and disarms combat, preventing stale engagement with healing off.
