...
`physical` and `master` tactical profiles also select the composition's
character/job-specific weapon and enable AutoWS2. PartyStart never targets,
moves, engages, or arms PartyCombat.
...
Choosing a tactical profile by itself (for example `//pstart physical`) applies
it to the last selected composition. `on` reapplies the last selected
composition/profile pair. Defaults are `progression/master`. `off` and `stop`
...
- Job discovery is dynamic, but expected roles and physical/master offense are
...
- Sentinel is attempted only after Flash lands on the current target or after
  a five-second establishment fallback. Rampart requires at least two party
  members below 90% with the lowest below 85%. Palisade requires recent HP
  pressure on the PLD. The controller never overlaps these three automated
  cooldown buffs.
