...
PartyStart never targets, moves, engages, or arms PartyCombat. A validated
profile supplies PartyCombat's required runtime-readiness policy; explicitly
stopping or reloading PartyStart revokes that readiness so combat cannot run
with the support controllers silently absent.
...
target nor move characters and never alter FastFollow. They now invalidate and
disarm PartyCombat, because continuing to fight with the PLD/RDM/DNC support
controllers off is unsafe. Reapply a PartyStart profile before the next
`//pc on`.
...
The Locus PLD controller uses the same proven Majesty-heal thresholds and MP
policy as `apexbats`; the RDM Convert fallback and DNC emergency Waltz policy
are likewise shared. If AutoWS2, Roller2, songs, and healing are all absent at
once, that is an inactive profile—not a Locus-specific cure threshold. Use the
lightweight activation/verification script above before resuming pulls.
